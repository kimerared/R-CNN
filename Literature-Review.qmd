...
Selective search employs a Hierarchical Grouping algorithm to detect objects with diverse characteristics such as scales, colors, textures, and enclosures. Initially, the method utilizes the Felzenszwalb and Huttenlocher @felzenszwalbmethod approach to segment the image into regions and obtain a set of initial regions. The grouping algorithm is detailed in the provided Algorithm.
...
![Flowchart of R-CNN](images/flowcharts/RCNN-flowcharts.png){style="text-align:center;" fig-alt="R-CNN Flowchart" fig-align="center"}
...
![Flowchart of Fast R-CNN](images/flowcharts/fast-RCNN-flowcharts.png){style="text-align: center;" fig-alt="Fast R-CNN Flowchart" fig-align="center"}

{{< pagebreak >}} {{< pagebreak >}}

### RoI Pooling Layer

The RoI pooling layer employs max pooling to transform features within any valid region of interest into a compact feature map with a fixed spatial extent of H × W (e.g., 7 × 7) @girshick2015fastrcnn, where H and W are layer hyper-parameters independent of any specific RoI.
...
![Faster R-CNN different stages @ren2015fasterrcnn](images/architecture/RPN-diagram.png){fig-alt="RPN diagram" fig-align="center" style="text-align:center;" width="295"}
...
![Feature Pyramid Network (FPN) @lin2016feature](images/architecture/pyramid-network-diagram.png){style="text-align: center;" fig-alt="Feature Pyramid Network (FPN)"}
...
![Flowchart of Faster R-CNN @ren2015fasterrcnn](images/flowcharts/faster-RCNN-flowcharts.png){style="text-align:center;" fig-alt="Faster R-CNN Flowchart" fig-align="center"}
...
![Mask R-CNN framework with RoIAlign @he2020maskrcnn](images/architecture/Mask-R-CNN.png){style="text-align:center;" fig-alt="Mask R-CNN framework with RoIAlign" fig-align="center"}
...
YOLO (You Only Look Once) is a popular object detection algorithm that revolutionized the field of computer vision due to its speed and accuracy @redmon2018yolov3. YOLO processes images in a single pass through a neural network, enabling real-time object detection. YOLOv8 is one of the iterations of the YOLO algorithm, incorporating various improvements over its predecessors to enhance performance and efficiency.

### Architecture

The architecture of YOLO consists of a backbone network, detection head, and output layers. The backbone network, typically based on Darknet or CSPDarknet, extracts features from the input image @bochkovskiy2020yolov4. These features are then processed by the detection head, which predicts bounding boxes, confidence scores, and class probabilities for detected objects. Earlier YOLO models utilizes anchor boxes @bochkovskiy2020yolov4 to improve localization accuracy and efficiency. Finally, the output layers produce the final detection and confidence scores.

![YOLOv8 architecture @zhang2023dcf-yolov8](images/architecture/yolov8-architecture.jpg){style="text-align:center;" fig-alt="Yolov8 architecture" fig-align="center"}

### Anchor-Free Approach

YOLOv8 employs an anchor-free approach @yolov8_ultralytics, eliminating the need for predefined anchor boxes. This simplifies the training process and improves model flexibility.
...
YOLO incorporates advanced techniques such as focal loss, which helps to address class imbalance and improve object detection accuracy @tian2019fcos.

### Real-Time Performance

YOLOv8 is optimized for real-time object detection applications @yolov8_ultralytics, offering fast inference speeds without compromising accuracy.
...
The main difference between YOLOv8 and Mask R-CNN is the region proposal network. YOLOv8 is an anchor free model so it does not need a region proposal network. Another difference between the two models is that Mask R-CNN is a two stage model and YOLOv8 is a one stage model. The first stage of Mask R-CNN is the application of the region proposal network and the second stage executes “bounding box regression, classification, and mask prediction” @ameli2024.

On the other hand, YOLOv8 has one stage that consists of “directly predicting bounding boxes and class labels for objects in an image” @ameli2024. YOLOv8 and Mask R-CNN also differ in their loss functions. YOLOv8 uses complete intersection over union loss, distribution focal loss, and binary cross entropy. Mask R-CNN uses classification loss, regression loss, and mask loss. In addition YOLOv8 is best used for real time object detection with low latency @ameli2024. 
